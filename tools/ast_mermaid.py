...
Output formats:
  markdown (default)   One ```mermaid block per diagram. Renders on GitHub, in editors, etc.
  html (--format html, or -o report.html)   A single self-contained page: mermaid.js from a
           CDN renders every diagram, with a live title filter and per-diagram **Download SVG
           / Download PNG** buttons. No build step, no local dependency.
  images (--images DIR)   Render each diagram straight to a file (png/svg/pdf) via mermaid-cli
           (`mmdc`), if it is installed. Skipped with a note if it is not.
...
No third-party Python dependencies - stdlib only. HTML output pulls mermaid.js from a CDN at
view time; --images shells out to the optional `mmdc` command.
...
Usage:
    ...
    python ast_mermaid.py wraithguard/ --graphs all -o report.html        # self-contained page
    python ast_mermaid.py wraithguard/ --graphs deps --images diagrams/    # png files via mmdc
"""
...
def _slug(title: str, i: int) -> str:
    """A filesystem/anchor-safe name for a diagram, e.g. 'call-graph-conflicts-py'."""
    s = re.sub(r"[^0-9a-zA-Z]+", "-", title).strip("-").lower()
    return s or f"diagram-{i}"


# One page, no build step: mermaid.js renders every diagram from a CDN, a search box filters
# by title, and each diagram gets client-side SVG/PNG download buttons (the SVG is serialised
# straight from the DOM; the PNG is that SVG rasterised onto a 2x canvas). Tokens are replaced
# rather than .format()'d so the CSS/JS braces need no escaping.
_HTML_PAGE = """<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>AST -> Mermaid Flowcharts</title>
<style>
  :root { color-scheme: light dark; }
  * { box-sizing: border-box; }
  body { font-family: system-ui, -apple-system, Segoe UI, sans-serif; margin: 0; background: #f6f7f9; color: #1a1a1a; }
  header { padding: 14px 24px; background: #1e1e1e; color: #eee; position: sticky; top: 0; z-index: 5; box-shadow: 0 1px 6px rgba(0,0,0,.25); }
  header h1 { margin: 0; font-size: 17px; font-weight: 600; }
  header h1 span { opacity: .55; font-weight: 400; }
  header input { margin-top: 8px; width: min(440px, 92%); padding: 7px 11px; border-radius: 7px; border: 1px solid #555; background: #2d2d30; color: #eee; font-size: 14px; }
  main { padding: 24px; max-width: 1240px; margin: 0 auto; }
  .card { background: #fff; border: 1px solid #e2e4e8; border-radius: 10px; margin: 0 0 24px; overflow: hidden; box-shadow: 0 1px 3px rgba(0,0,0,.06); }
  .card > h2 { margin: 0; padding: 11px 16px; font-size: 14px; font-weight: 600; background: #eef0f3; border-bottom: 1px solid #e2e4e8; display: flex; align-items: center; gap: 8px; }
  .card > h2 .btns { margin-left: auto; display: flex; gap: 6px; }
  .card button { font-size: 12px; padding: 4px 10px; border: 1px solid #c3c7cd; border-radius: 6px; background: #fff; color: inherit; cursor: pointer; }
  .card button:hover { background: #f0f2f5; }
  .diagram { padding: 16px; overflow: auto; }
  .mermaid { display: flex; justify-content: center; min-height: 40px; }
  /* Let the card show through instead of a hard white fill -- the white-out that
     bites the markdown diagrams in a dark viewer (only Obsidian papers over it). */
  .mermaid svg { background: transparent !important; }
  .hidden { display: none; }
  @media (prefers-color-scheme: dark) {
    body { background: #141416; color: #e6e6e6; }
    .card { background: #1e1e1e; border-color: #333; box-shadow: none; }
    .card > h2 { background: #26262a; border-color: #333; }
    .card button { background: #2d2d30; color: #e6e6e6; border-color: #444; }
    .card button:hover { background: #37373b; }
  }
</style>
</head>
<body>
<header>
  <h1>AST -&gt; Mermaid Flowcharts <span>&middot; __COUNT__ diagram(s)</span></h1>
  <input id="filter" type="search" placeholder="Filter diagrams by title..." oninput="filterCards(this.value)">
</header>
<main id="main">
__CARDS__
</main>
<script type="module">
  ...
  mermaid.initialize({ startOnLoad: true, securityLevel: "loose",
    theme: dark ? "dark" : "default", flowchart: { useMaxWidth: true } });
</script>
<script>
  ...
  function _svg(card) { return card.querySelector(".mermaid svg"); }
  function _download(blob, name) {
    const a = document.createElement("a");
    a.href = URL.createObjectURL(blob); a.download = name; a.click();
    setTimeout(() => URL.revokeObjectURL(a.href), 1500);
  }
  function saveSvg(card, name) {
    const svg = _svg(card); if (!svg) return;
    const data = new XMLSerializer().serializeToString(svg);
    _download(new Blob([data], { type: "image/svg+xml;charset=utf-8" }), name + ".svg");
  }
  function savePng(card, name) {
    const svg = _svg(card); if (!svg) return;
    ...
    img.onload = () => {
      ...
      canvas.toBlob(b => b && _download(b, name + ".png"), "image/png");
    };
    img.onerror = () => alert("Could not rasterise this diagram to PNG; use SVG instead.");
...
def render_html(sections: list[tuple[str, str]]) -> str:
    """A single self-contained HTML page rendering every diagram via mermaid.js (CDN)."""
    cards = []
...
def write_images(sections: list[tuple[str, str]], out_dir: str, image_format: str) -> int:
    """Render each diagram to an image file via mermaid-cli (`mmdc`), if it is installed.

    Returns the number of images written. Prints a note and returns 0 when `mmdc` is not
    on PATH -- the feature is a bonus for those who have it, never a hard requirement.
    """
    import shutil
    import subprocess
    import tempfile
    ...
    if not mmdc:
        print(
            "note: --images needs mermaid-cli. Install it with "
            "`npm install -g @mermaid-js/mermaid-cli`, then re-run. Skipping images.",
            file=sys.stderr,
        )
        return 0
    ...
    for i, (title, body) in enumerate(sections, 1):
        ...
        try:
            ...
            if proc.returncode == 0:
                ...
                print(f"  FAILED {name}: {msg[-1] if msg else 'mmdc error'}", file=sys.stderr)
        except OSError as e:
            print(f"  FAILED {name}: {e}", file=sys.stderr)
        finally:
            if src:
                Path(src).unlink(missing_ok=True)
    print(f"Wrote {written}/{len(sections)} image(s) to {out}/", file=sys.stderr)
    return written
...
def main():
    ...
    # sequence. That's invisible when writing via -o/--output (write_text always forces
    # UTF-8), but if the report is piped instead (`python ast_mermaid.py ... > report.html`),
    # print() goes through that legacy codepage and the file ends up with a stray 0x97
    # byte sitting inside a document declared <meta charset="utf-8">, which browsers then
    # render as a mangled character. Reconfiguring here makes stdout/stderr UTF-8
    # regardless of console codepage, so redirected output is safe too.
    ...
    # Render straight to image files too, if asked (independent of the text report).
    if args.images:
        write_images(sections, args.images, args.image_format)

    # Decide the text report's format: explicit --format wins, else infer from -o's
    # extension, else markdown.
    out_path = Path(args.output) if args.output else None
    fmt = args.format
    if fmt is None:
        fmt = "html" if (out_path and out_path.suffix.lower() in (".html", ".htm")) else "md"

    content = render_html(sections) if fmt == "html" else render_markdown(sections)

    if out_path:
        out_path.write_text(content, encoding="utf-8")
        print(f"Wrote {len(sections)} diagram(s) to {out_path} ({fmt})", file=sys.stderr)
    elif not args.images:
        # No file and no images requested: the report goes to stdout as before. If only
        # --images was given, we stay quiet on stdout rather than dumping markdown.
        print(content)
