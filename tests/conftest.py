"""Shared pytest fixtures and helpers for the Wraithguard Toolkit test suite.
...
ENGINE_PATH = REPO_ROOT / "wraithguard_toolkit.py"


def _load_engine() -> types.ModuleType:
    """Import wraithguard_toolkit.py by path, without needing it on sys.path."""
    spec = importlib.util.spec_from_file_location("wraithguard_toolkit", ENGINE_PATH)
    if spec is None or spec.loader is None:  # pragma: no cover - defensive
        raise RuntimeError(f"cannot load engine from {ENGINE_PATH}")
    module = importlib.util.module_from_spec(spec)
    sys.modules.setdefault("wraithguard_toolkit", module)
