...
SOURCE_FILES = sorted(
    ...
    for path in [
        ...
        *(PROJECT_ROOT / "wraithguard").rglob("*.py"),
...
def test_pep440_version_is_valid() -> None:
    ...
    assert re.match(pattern, wraithguard.__version__), wraithguard.__version__
...
def test_pep621_metadata_present_and_consistent() -> None:
    ...
    assert project["version"] == wraithguard.__version__
...
def test_public_api_all_names_resolve() -> None:
    ...
    ``AttributeError`` on ``from wraithguard import *`` -- and, less
    obviously, a silent hole in what the docs promise callers can import.
    """
    import wraithguard

    missing = [name for name in wraithguard.__all__ if not hasattr(wraithguard, name)]
...
def test_pep484_mypy_gate_is_configured() -> None:
    ...
    ``wraithguard`` dropped from the checked set.
    ...
    assert "wraithguard" in files, "mypy must check the whole wraithguard package"
    assert "wraithguard_toolkit.py" in files, "mypy must check the engine/CLI script"
...
@pytest.mark.parametrize(
    "path",
    [p for p in SOURCE_FILES if "wraithguard" in p.parts],
    ids=lambda p: p.name,
)
...
def test_pep420_every_package_directory_is_explicit() -> None:
    ...
    """
    for directory in (PROJECT_ROOT / "wraithguard").rglob("*"):
...
def test_declared_packages_match_what_exists() -> None:
    ...
    actual = {"wraithguard"} | {
        "wraithguard." + directory.name
        for directory in (PROJECT_ROOT / "wraithguard").iterdir()
        if directory.is_dir()
...
def test_pep561_py_typed_marker_present() -> None:
    ...
    """
    assert (PROJECT_ROOT / "wraithguard" / "py.typed").is_file()
...
class TestNoReExportShim:
    """The engine must not stand in front of ``wraithguard/``.

    Until 3.0, ``wraithguard_toolkit.py`` imported 36 names purely so that
    ...
    def _engine_imports() -> dict[str, str]:
        """Map every name the engine imports from ``wraithguard/`` to its module.

        Returns:
            ``{bound_name: source_module}``, using the bound (possibly aliased)
            name, since that is what a ``core.<name>`` reference would resolve.
        """
        tree = ast.parse((PROJECT_ROOT / "wraithguard_toolkit.py").read_text(encoding="utf-8"))
        return {
            alias.asname or alias.name: node.module or ""
            for node in tree.body
            if isinstance(node, ast.ImportFrom) and (node.module or "").startswith("wraithguard")
    ...
    def test_no_caller_reaches_a_library_name_through_the_engine(self) -> None:
        ...
        ``wraithguard.sort.build_and_sort`` is the shim: two obvious ways to
        ...
        assert not offenders, (
            "these call sites reach a wraithguard name through the engine; import it "
    ...
    def test_every_engine_import_is_actually_used(self) -> None:
        ...
        """
        source = (PROJECT_ROOT / "wraithguard_toolkit.py").read_text(encoding="utf-8")
...
def test_gettext_marker_is_never_shadowed_by_unpacking() -> None:
    ...
    is a live bug in any module that imports ``_`` from ``wraithguard``: it
    ...
    for path in PROJECT_ROOT.rglob("*.py"):
        ...
        imports_marker = any(
            isinstance(node, ast.ImportFrom)
            and (node.module or "").startswith("wraithguard")
...
def _dark_palette_keys() -> set[str]:
    ...
    ``wraithguard/gui/theme.py`` imports :mod:`tkinter` at module level and the
    ...
    """
    tree = _parse(PROJECT_ROOT / "wraithguard/gui/theme.py")
    for node in ast.walk(tree):
    ...
    message = "DARK palette not found in wraithguard/gui/theme.py"
