        ...
        try:
            with redirect_stdout(writer.as_stream()), redirect_stderr(writer.as_stream()):
                ...
                if server is not None:
                    ...
                    # Every key registered below, for the prewarm pass after
                    # build_cell_viewer_page() -- see its call further down.
                    tex_keys: list[str] = []
                    ...
                    def publish_texture(resolved: Resolved) -> dict[str, str] | None:
                        ...
                        key = f"t{next(tex_counter)}.png"
                        url = session.register_lazy(key, producer)
                        tex_keys.append(key)
                        made = {"url": url}
                    ...
                    # Fire-and-forget: decodes tex_keys ahead of the browser's own
                    # requests for them, on the same worker count already computed
                    # above for mesh parsing (free-threading-gated). See
                    # ViewerServer.prewarm for why this exists -- the browser's own
                    # per-origin connection cap otherwise leaves cores idle during
                    # a cell's texture decodes. Never blocks: the page is already
                    # servable the instant this call returns.
                    session.prewarm(tex_keys, workers=workers)
                    opened = ("url", url)
