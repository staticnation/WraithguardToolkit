...
exhausts memory. This is the only path the cell viewer uses; the earlier
baked-per-placement path (``build_scene``/``BuiltScene``) was removed once
nothing called it any more -- see ``UNBAKE_MIGRATION.md``.
...
def build_instanced(
    ...
    *,
    ...
    Loads every unique model once and records the per-placement transforms as
    matrices, rather than copying and re-transforming vertices per placement.
    A cell that places the same crate three hundred times then carries one
    crate mesh and three hundred matrices, not three hundred crates -- the
    difference between a page that opens and one that exhausts memory.
