...
the geometry-retention path in `mlox_subset/nif/reader.py` all exist to get
...
If the viewer parses with `tes3` and the scan parses with `mlox_subset.nif`,
...
Put the built artifact beside three.js in `mlox_subset/nif/assets/`, with its
