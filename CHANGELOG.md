...
- **The download buttons can create their file on a first run.** Clicking
  **Update...** for `plugin-order.yml` with the path blank now asks where to save
  it and remembers the choice, rather than refusing until you Browse. **Update
  Rules...** does the same: with no rule files set it asks for a folder, puts
  `mlox_base.txt` and `mlox_user.txt` there, adds them to the list, and downloads
  them -- so a fresh install can fetch its rules without hunting one down by hand
  first.

- **Pick a plugin, then pick the fields it should win across every record it
  defines -- a "Merge this plugin's fields..." button in the Plugin view.**
  Doing this by hand for a mod that fixes the same fields in a hundred records
  -- a rebalance that changes every weapon's weight, or restores every
  creature's name -- is a hundred clicks. Select a plugin in the tree, click the
  button, and pick which of its fields to take: each is queued from that plugin
  in every record it defines. The field picker lists only the fields this plugin
  would actually change, each with a count of how many records it would patch and
  a preview of the plugin's own value for that field (so you can see what you are
  forcing, not just its name), and shows a running total as you select. It stays the smallest patch that
  changes what you asked to change -- a record is included only when the plugin
  defines it, does *not* already win it, and gives a value that differs from the
  current winner's (equal values need no patch); identity fields (`type`, `id`,
  the cell grid) are never offered. The plugin is read on a background thread
  with the same batched, per-plugin-locked reader the tree colouring uses, so
  the window stays responsive even on a large mod. Everything else reuses the
  existing machinery: the same `FieldChoice` decisions into the same queue, the
  same builder window and writer, so a bulk decision is as reviewable and
  reversible as a single one, and no mod is ever modified. The judgement of
  *which* records is a pure, display-free function, `bulk_field_choices` in
  `wraithguard/patch/bulk.py` (with `BulkRecord` and `BulkFieldDecision`),
  tested exhaustively without tkinter.

- **Define your own value for a field in the patch builder -- a "Define
  value..." button beside "Merge field...".** Until now a conflicting field
  could be settled two ways: carry a whole record, or take the field from
  another plugin. This adds a third: type the value directly, so a patch can
  hold a number or string that *no* plugin in the conflict uses -- a compromise
  weight, a corrected health, a name of your own. Select a record and a field,
  click Define value, and give it one: the dialog picks an input to match the
  field's type -- a checkbox for a flag, a spinbox for a number, a multi-line box
  for a list or group, and for a string an editable dropdown of the field's known
  enum variants and the values the other plugins in the conflict use (so you can
  pick a valid variant or type your own) -- so the value comes back as the right
  JSON type (an integer stays an integer, a flag a boolean) and one tes3conv
  cannot rebuild is refused with a message rather than written and discovered
  later as a broken patch. Identity fields (`type`, `id`, the cell
  grid) are off-limits -- retyping those makes a *different* record, not a
  patched one. Everything stays additive: the value is written verbatim into one
  new plugin that loads last, and deleting it restores the previous behaviour
  exactly. A literal belongs to no plugin's master list, so unlike a taken field
  it is never reindexed -- a typed reference list is written as given. New model:
  `FieldValue`/`Choice` in `wraithguard/patch/merge.py`, the type-matching
  parser `parse_field_value` in `wraithguard/patch/values.py`; the queue,
  builder window and diff dialog carry both kinds of decision. The string
  dropdown's variants come from `wraithguard/patch/enum_data.py`, generated by
  `tools/gen_tes3_enums.py`: the vanilla masters' tes3conv output supplies the
  JSON field names, and the `tes3` crate's `enums.rs` supplies each enum's
  complete, authoritatively-spelled variant list (so a variant no vanilla record
  uses, like `SpellType::Curse`, is still offered). 42 enum fields --
  weapon/armor/clothing/apparatus/creature/book type, spell and enchant type,
  school, skill, attribute, specialization, biped and body parts, dialogue
  filters, and more -- plus the conflict's own values, merged in. The input for a
  field is chosen by the field's *declared* type, read from the `tes3` crate's
  record structs by `tools/gen_tes3_fieldtypes.py` into
  `wraithguard/patch/field_types.py` (43 record types, keyed by tes3conv `type`
  and JSON path): a `u16` like `Weapon.data.health` gets a spinbox bounded to its
  real range (0-65535, and a value outside it is refused), an `f32` a spinbox, an
  enum a dropdown, a list a JSON box; a `bitflags` field like `data.flags` gets a
  checkbox per flag (`SILVER`, `IGNORES_NORMAL_WEAPON_RESISTANCE`, ...) composed
  back to tes3conv's ` | `-joined string, preserving any unnamed `0x..` bits it
  does not cover. Where the crate does not describe a field the input still falls
  back to the current value's type, so nothing is a dead end.

- **See every quest's journal chain across your whole load order -- a "Journal
  chains..." button in the Conflicts window.** A read-only window that resolves
  each quest and lists its journal stages sorted by journal index rather than
  file order, with the winning plugin's text for each stage. The point is the row
  *under* each stage: what actually sets it. That source is followed generically
  -- a standalone `SCPT` script, another stage's own result script, or, crucially,
  an ordinary NPC dialogue response whose result script happens to set the journal
  (where most real quest progression lives, since the journal topic's own entry is
  often empty). Selecting a stage or a "Set by"/"Also sets"/"Target stage" row
  shows, in a wrapping detail pane, its raw enclosing condition verbatim (never
  evaluated -- the same restraint the script disassembler applies), the dialogue
  requirements that gate it (who must be speaking, what disposition, rank or
  filter must hold, via the same `condition_lines` the field-diff viewer already
  uses), the full result script, and everything else that script does at that
  moment -- item grants, disposition changes, other scripts starting -- captured
  generically rather than matched against a curated list that would always miss
  something. A row that resolves to a real stage is a questline link and can be
  double-clicked to jump the tree there, including across quests. The whole load
  order is read once, in the background, when the window opens (not per group or
  per click, since resolving and scanning are cheap once the records are in hand);
  it needs `tes3conv` for that read, the same as the rest of the Conflicts
  tooling. New: `wraithguard/patch/journal.py` (`resolve_journals`) resolves each
  quest's surviving stages, `wraithguard/patch/journal_scripts.py` extracts the
  call sites and effects from scripts and dialogue, and
  `wraithguard/gui/journalview.py` (`JournalViewMixin`) is the window.

- **Pull your openmw.cfg's unmanaged plugins and data paths in as a subset to
  sort -- a new `--subset-from-cfg` flag, and a "Pull unmanaged (orphan) entries
  from openmw.cfg" checkbox in the GUI.** Until now the subset to sort came from
  a folder scan, a `--subset-file`, or a `--customizations` TOML; this adds a
  fourth source: the cfg itself. Every `content=` plugin and `data=` path already
  in openmw.cfg that nothing accounts for -- neither on the curated list
  (`--plugin-order-yml` + `--list-name`) nor in your customizations -- is
  collected, in its existing cfg order (kept as the frozen starting order until
  the sort repositions it), and handed to the sorter as `already_present`
  entries it moves within the frozen order rather than new insertions. Base game
  masters (Morrowind/Tribunal/Bloodmoon.esm), the base `Data Files` folder, and
  anything the cfg declares as groundcover are never pulled. It can be the only
  subset source or combined with the others; `data=` orphans are only positioned
  with `--sort-data-paths` (otherwise just listed). This is the same "orphan"
  set the read-only `[ORPHAN]` warning already reported -- now usable as an
  input rather than only a note. Prefer exporting the result to a
  customizations.toml, since momw-configurator rebuilds openmw.cfg and would
  overwrite a direct `--write-cfg` patch. Core in
  `wraithguard/configurator/cfglines.py` (`orphan_cfg_entries`,
  `curated_covers`, `is_base_data_path`, `BASE_GAME_MASTERS`); pipeline stage
  `_pull_cfg_orphans` in `compute_plan`.

### Fixed

- **"Pull unmanaged (orphan) entries from openmw.cfg" no longer marks every
  `data=` path as moved.** With a curated list (a big MOMW total-overhaul, say)
  the option turned nearly the whole Data path panel red and reordered the VFS.
  The cause: a `data=` path is already in the cfg's own `data=` order, but the
  orphan pull was re-feeding every path a curated list did not explicitly
  declare back in as a *new* insert -- and `plugin-order.yml` has no data-path
  concept, so with a list in play that is essentially every path. The pull now
  takes only orphan `content=` plugins (where the curated set reliably tells a
  loose plugin from a listed one) and leaves `data=` exactly as `openmw.cfg` has
  it. The option is relabelled "Pull unmanaged (orphan) **plugins**" to match.

- **A folder name containing `&` no longer writes its `data=` path twice.**
  Reported from the field: a data path whose folder name held an `&` was emitted
  twice -- once with a single `&`, once doubled to `&&` -- in both `openmw.cfg`
  and the customizations TOML. Inside a *quoted* `data=` value OpenMW treats `&`
  as its escape character, so a literal `&` must be written `&&` (and a literal
  `"` as `&"`); the tool never escaped or unescaped it. A path already present in
  the cfg in escaped form (`...&& Friends`) therefore never matched a new insert
  carrying the real single-`&` path, the duplicate-check missed it, and a second
  line was added. `data=` values are now unescaped on read and escaped on write
  whenever the value is quoted (a bare, unquoted value stays literal, since `&`
  is not special there), so the two forms of the same folder compare equal and
  the duplicate is skipped -- and a genuinely `&`-bearing path is now written in
  the correct escaped form OpenMW actually reads. New `escape_cfg_value` /
  `unescape_cfg_value` in `wraithguard/configurator/cfglines.py`, applied through
  `extract_data_path_value`, `format_data_line` and `normalize_data_path`, so the
  cfg apply and the TOML emit both benefit.

- **The `openmw.cfg` file picker's extension filter no longer matches nothing on
  Linux.** The case-insensitive filter helper (which rewrites `*.cfg` to
  `*.[cC][fF][gG]` so Tk's X11 dialog stops hiding upper-case names) was being
  applied twice on some pickers -- once at the call site and once inside the path
  widget -- doubling the glob into `*.[[cC]][[fF]]...`, which matched no file, so
  `openmw.cfg` could only be selected by switching the dialog to "All files".
  The pickers now pass their filters raw and let the widget wrap them exactly
  once, and `case_insensitive_filetypes` is now idempotent -- a pattern that
  already carries a `[..]` class is left alone -- so even a future double-wrap
  cannot reintroduce the doubled form.

- **Cleaned plugins are no longer misreported as orphans (and no longer risk
  being un-frozen).** When a plugin the curated list flags `needs_cleaning` is
  fetched through umo it arrives cleaned and renamed with a `clean_` prefix --
  the list names `sm_golden_mask.ESP`, the file on disk is
  `Clean_sm_golden_mask.ESP` -- so it matched nothing on the list and was
  wrongly reported `[ORPHAN]` (and would have been scooped into the sortable
  subset by the new cfg pull). Orphan detection now goes through
  `curated_covers`: an exact list match first (which also covers yml entries
  that already carry the `clean_` prefix in their own `file_name`), else a
  single leading `clean_` is stripped and the base must be BOTH on the list AND
  flagged `needs_cleaning` for the match to count. Only one prefix is stripped
  deliberately -- `clean_clean_` never appears on the list, and stripping every
  prefix would risk mistaking a genuinely custom `Clean_MyMod.esp` for a list
  item -- so a real custom stays an orphan while a legitimately cleaned list
  plugin is recognised as managed, kept frozen, and never pulled into the
  customizations.

### Internal

- **Release artifacts are signed with minisign in CI.** Each OS build now signs
  its `.exe`/binary/`.zip` and attaches a `.minisig` to the Release, via a new
  pure-Python `tools/sign_release.py` (the `py-minisign` package -- no C or Rust
  toolchain on the runner). The signing key comes from the `MINISIGN_SECRET_KEY`
  / `MINISIGN_PASSWORD` Actions secrets; with them unset the step is skipped
  rather than failing. Verify a download with `minisign -Vm <file> -p minisign.pub`
  (README: "Verifying a download").

- **A pure-Python reader and writer for TES3 plugins has begun, in
  `wraithguard/esp/`.** The toolkit's authoritative plugin reader is
  `tes3conv`, a console binary it shells out to -- a subprocess to locate and
  bundle, a window it spawns on Windows, and a reader that refuses a whole file
  on one record it does not know (OpenMW's `LUAL`, which is why
  `wraithguard/land/native.py` had to exist). This is the start of reading and
  writing the format *in process* instead, a faithful port of the `tes3` Rust
  crate's `esp` library. The foundation is in: the binary layer
  (`wraithguard/esp/io.py` -- little-endian primitives, four-byte tags,
  length-prefixed null-terminated Windows-1252 strings), the sixteen-byte record
  framing and tag dispatch (`record.py`, `plugin.py`), and the crate's complete
  enum and bitflags tables generated from its source by `tools/gen_esp_types.py`
  into `wraithguard/esp/enums.py` and `flags.py` (29 enums, 20 flag sets; an enum
  coerces an unknown value to its default as the crate does, a flag keeps bits no
  named flag covers). **All forty-three TES3 record types are ported**,
  end-to-end and verified byte-for-byte -- a record read and written again is
  exactly its original bytes:
  `WEAP` (weapon), `STAT` (static), `ACTI` (activator), `DOOR`, `APPA`
  (apparatus), `LOCK` (lockpick), `PROB` (probe), `REPA` (repair item), `BOOK`,
  `MISC` (misc item), `LIGH` (light), `INGR` (ingredient), `ALCH` (potion),
  `ARMO` (armor), `CLOT` (clothing), `SOUN` (sound), `SNDG` (sound generator),
  `SSCR` (start script), `GLOB` (global variable -- a typed value stored as an
  `f32`, read through its `float`/`long`/`short` type as the engine does, with a
  `NaN` reading as zero for the one such global `Morrowind.esm` ships), `GMST`
  (game setting, a string/float/integer value keyed by its subrecord tag), `SKIL`
  (skill), `LTEX` (landscape texture), `BODY` (body part), `CLAS` (class), `BSGN`
  (birthsign, its granted spells fixed-width `NPCS` fields), `ENCH` (enchantment),
  `SPEL` (spell), `LEVI` (leveled item list) and `LEVC` (leveled creature list,
  both interleaving id and level subrecords), `MGEF` (magic effect), `CONT`
  (container, with a fixed-id inventory), `REGN` (region, whose weather block
  reads an 8- or 10-byte form and always writes 10) and `FACT` (faction, a
  240-byte block of ten per-rank requirements plus `ANAM`/`INTV` reactions),
  `RACE` (race), `TES3` (the plugin header itself -- its version, type, author
  and description, record count, and master list -- so a whole plugin, header and
  content, round-trips as one), `SCPT` (script -- name, counts, and the variable
  table, bytecode and source kept verbatim), `PGRD` (path grid -- its points,
  their padding, and the connection table), `DIAL` (dialogue topic), `LAND`
  (landscape -- its terrain grids kept as raw bytes and written only as the
  landscape flags call for), `NPC_` (a non-player character -- its two-form stats
  block, a flag word that packs the NPC flags with a blood type, an inventory,
  spells, and the shared AI data, packages and travel destinations) and `CREA`
  (a creature -- the same AI machinery with its own 96-byte data block and a
  clamped, default-dropping scale). The AI data, the five AI package kinds
  (`AI_T`/`AI_W`/`AI_E`/`AI_F`/`AI_A`, two of which trail an out-of-size cell
  name) and travel destinations live in `records/_ai.py`, shared by both. And
  `CELL` (a cell -- its header plus every object reference placed in it: the
  reference sub-record with its two dozen optional overrides in `records/`
  `reference.py`, moved-reference framing, and the marker separating persistent
  from temporary references, all round-tripping in file order) and `INFO` (a
  dialogue response -- its linked-list ids, speaker filters with float-or-integer
  values, quest state, and text stored without a null terminator). Alchemy,
  ingredient, enchantment and spell carry the shared
  `ENAM` effect block (`records/effect.py`); armor and clothing carry lists of
  the `INDX`/`BNAM`/`CNAM` biped-object group (`records/bipedobject.py`), whose
  read looks ahead for each slot's optional male and female meshes -- for which
  the reader gained a `try_tag` that consumes a tag only when it matches.
  `records/_common.py` holds the moves they share (the `DELE` deletion marker,
  optional strings omitted when empty, fixed data blocks sized and checked, an
  unknown tag refused by name). A record type the library does not recognise is
  still preserved verbatim rather than refused, so an OpenMW-only or future record
  never stops a read. `tes3conv` remains the tool's reader for the full
  record-by-record conflict scan, but the header read is now wired to this
  library: a new `read_header` reads just the first (`TES3`) record -- so a large
  plugin is not read whole, and a file whose later records `tes3conv` would refuse
  still yields its header -- and `read_plugin_masters`,
  `read_plugin_masters_with_sizes` (the sort engine's dependency edges and the
  missing-master / master-size checks) and `read_plugin_description` (the mlox
  `[VER]`/`[DESC]` predicates) now read through it. That replaces three
  hand-rolled byte scans that decoded as `latin-1`; the library decodes the
  format's own cp1252, so a description with a smart quote or an accented name
  comes back correct where the old scan turned bytes 0x80-0x9F into control
  characters. Each keeps its byte scan as a fallback for a header too malformed
  for the strict reader, so nothing regresses on an odd file.

- **`PatchBuilderMixin` now declares the host-class members it borrows.** The
  patch-write path calls `_ensure_conflict_session`, `_apply_exclusions` and
  `order_panel`, which live on sibling mixins of the composed window; the
  `TYPE_CHECKING` declaration block that tells the type checker about such
  members was missing these three (every other mixin that uses them declares
  them). A clean type-check run surfaced it; the declarations are added, matching
  the pattern in `journalview.py` and `conflicts.py`. No behaviour change.

- **`test_the_app_keeps_a_reference_to_its_icon_photo` is deterministic across
  platforms.** The window icon now tries `iconbitmap` first and only decodes a
  `PhotoImage` as a fallback (the X11 path, where `.ico` loading is a silent
  no-op), so on Windows -- where `iconbitmap` succeeds -- `_icon_photo` is never
  set and the test's "the reference survives GC" guarantee has nothing to check.
  The test now forces `iconbitmap` to raise (standing in for X11) so it
  exercises the fallback and its reference-keeping guarantee on every platform,
  instead of passing on Linux/xvfb and failing on a real Windows display.
