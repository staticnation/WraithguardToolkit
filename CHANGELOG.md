...
not on disk. The suite grew to **1,261 tests** over the course of the split
...
- **Translation marking, complete.** `mlox_subset/i18n.py` provides gettext
  ...
  (**431 messages**), regenerated by the new **`tools/make_pot.py`**:
...
- **mypy now gates the entire codebase** — all **54** files, up from 28.
...
- **A native TES3 record schema** (`mlox_subset/tes3fields/schema.py`, generated
  by `tools/gen_tes3_schema.py`): 45 record types, 309 subrecords, 61 with parsed
  struct layouts, built from UESP's format-page tables. 55 of those layouts
  declare a plain byte count, and all 55 agree with the sum of their parsed
